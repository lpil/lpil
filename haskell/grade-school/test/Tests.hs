...
specs = do
  ...
  it "add student" $ sorted (add 2 "Aimee" empty) `shouldBe` [(2, ["Aimee"])]
  ...
  it "get students in a non-existent grade" $ grade 1 empty `shouldBe` []

  it "sorted school"
    $          sorted
                 ( fromList
                   [(4, "Jennifer"), (6, "Kareem"), (4, "Christopher"), (3, "Kyle")]
                 )
    `shouldBe` [ (3, ["Kyle"])

...
toDigits n
  | n <  0    = error "toDigits: negative number"
  | n == 0    = [0]
...
doubleSecond []        = []
doubleSecond [x]       = [x]
doubleSecond (x:x2:xs) = [x, x2 * 2] ++ doubleSecond xs
...
isValid x = rem (sumDigits . doubleSecond $ toDigitsRev x) 10 == 0

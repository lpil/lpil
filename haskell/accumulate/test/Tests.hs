...
main :: IO ()
main = exitProperly $ runTestTT $ TestList [TestList accumulateTests]
...
accumulateTests
  = [ testCase "empty accumulation" $ [] @=? accumulate square []
    , testCase "accumulate squares" $ [1, 4, 9] @=? accumulate square [1, 2, 3]
    , testCase "accumulate upcases" $ ["HELLO", "WORLD"] @=? accumulate
      (map toUpper)
      ["hello", "world"]
    ...
    , testCase "accumulate non-strict" $ ["nice work!"] @=? take
      1
      ( accumulate
        id
        ( "nice work!"

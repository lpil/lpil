...
#[test]
fn follow_instructions_left() {
    let robot = Robot::new(0, 0, Direction::North).instructions("L");
    assert_eq!((0, 0), robot.position());
    assert_eq!(&Direction::West, robot.direction());
}

#[test]
fn follow_instructions_right() {
    let robot = Robot::new(0, 0, Direction::North).instructions("R");
    assert_eq!((0, 0), robot.position());
    assert_eq!(&Direction::East, robot.direction());
}

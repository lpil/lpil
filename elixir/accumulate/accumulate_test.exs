...
defmodule AccumulateTest do
  ...
  describe "tail_accumulate" do
    test "accumulate empty list" do
      assert Accumulate.tail_accumulate([], fn(n) -> n * n end) == []
    end

    test "accumulate square numbers" do
      assert Accumulate.tail_accumulate([1, 2, 3], fn(n) -> n * n end) == [1, 4, 9]
    end

    test "accumulate upcased strings" do
      fun = fn(w) -> String.upcase(w) end
      assert Accumulate.tail_accumulate(["hello", "world"], fun) == ["HELLO", "WORLD"]
    end
    ...
    test "accumulate reversed strings" do
      ...
      words = ~w(the quick brown fox etc)
      expected = ["eht", "kciuq", "nworb", "xof", "cte"]
      assert Accumulate.tail_accumulate(words, fun) == expected
    end
    ...
    test "accumulate recursively" do
      ...
      fun = fn(c) -> for num <- nums, do: c <> num end
      expected = [["a1", "a2", "a3"], ["b1", "b2", "b3"], ["c1", "c2", "c3"]]
      assert Accumulate.tail_accumulate(chars, fun) == expected
    end
  end
end
